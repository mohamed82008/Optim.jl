...
KrylovTrustRegion(::Type{T}=Float64; initial_radius::Real = one(T),
                    max_radius::Real = T(100),
                    eta::Real = T(0.1),
                    rho_lower::Real = T(0.25),
                    rho_upper::Real = T(0.75),
                    cg_tol::Real = T(0.01)) where T =
...
function assess_convergence(state::KrylovTrustRegionState, d, options::Options{T}) where {T}
    ...
    # Relative Tolerance
    if abs(state.f_diff) < max(options.f_tol * (abs(value(d)) + options.f_tol), T(eps(abs(value(d)))+abs(state.f_x_previous)))
        f_converged = true
    end

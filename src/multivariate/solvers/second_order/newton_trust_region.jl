...
function check_hard_case_candidate(H_eigv, qg)
    ...
    while !hard_case_check_done
        ...
        elseif abs(H_eigv[1] - H_eigv[lambda_index]) > T(1e-10)
            # The eigenvalues are reported in order.
            hard_case_check_done = true
        else
            if abs(qg[lambda_index]) > T(1e-10)
                hard_case_check_done = true
                hard_case = false
            end
            lambda_index += 1
...
function solve_tr_subproblem!(gr::AbstractArray{T},
    ...
    if min_H_ev >= T(1e-8) && p_sq_norm(zero(T), 1, n, qg, H_eig) <= delta_sq
        ...
        lambda_lb = -min_H_ev + max(T(1e-8), T(1e-8) * (max_H_ev - min_H_ev))
        lambda = lambda_lb
        ...
        if hard_case_candidate
            ...
            if p_lambda2 > delta_sq
                ...
                lambda = lambda_lb + T(0.01) * (max_H_ev - min_H_ev)
            else
                hard_case = true
                reached_solution = true
        ...
        if !hard_case
            ...
            for iter in 1:max_iters
                ...
                if lambda < (lambda_lb + T(1e-8))
                    lambda = T(0.5) * (lambda_previous - lambda_lb) + lambda_lb
                end
    ...
    m = vecdot(gr, s) + T(0.5) * vecdot(s, H * s)
...
NewtonTrustRegion(::Type{T}=Float64; initial_delta::Real = one(T),
                    ...
                    eta::Real = T(0.1),
                    rho_lower::Real = T(0.25),
                    rho_upper::Real = T(0.75)) where T =
...
function initial_state(method::NewtonTrustRegion, options, d, initial_x)
    ...
    @assert(method.delta_hat > zero(T), "delta_hat must be strictly positive")
    ...
    @assert(method.rho_lower >= zero(T))
...
function update_state!(d, state::NewtonTrustRegionState, method::NewtonTrustRegion)
    ...
    if state.rho <= state.eta
        ...
        delta = T(0.25) * sqrt(vecdot(x_diff, x_diff))

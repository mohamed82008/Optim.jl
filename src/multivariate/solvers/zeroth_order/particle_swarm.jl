...
function update_state!(f, state::ParticleSwarmState{Tx,T}, method::ParticleSwarm) where {Tx,T}
    ...
    sigma_learn = 1 - (1 - T(0.1)) * state.iteration / state.iterations
...
function get_mu_1(f::T) where T
    if T(0) <= f <= T(0.4)
        return T(0)
    elseif T(0.4) < f <= T(0.6)
        return 5 * f - 2
    elseif T(0.6) < f <= T(0.7)
        return T(1)
    elseif T(0.7) < f <= T(0.8)
        return -10 * f + 8
    else
        return T(0)
    end
...
function get_mu_2(f::T) where T
    if T(0) <= f <= T(0.2)
        return T(0)
    elseif T(0.2) < f <= T(0.3)
        return 10 * f - 2
    elseif T(0.3) < f <= T(0.4)
        return T(1)
    elseif T(0.4) < f <= T(0.6)
        return -5 * f + 3
    else
...
function update_swarm_params!(c1, c2, w, current_state, f)
    ...
    delta_c1 = T(0.05) + rand(T) / 20
    delta_c2 = T(0.05) + rand(T) / 20

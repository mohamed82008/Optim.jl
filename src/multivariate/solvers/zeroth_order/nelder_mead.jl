...
AffineSimplexer(::Type{T}=Float64; a = T(0.025), b = T(0.5)) where T = AffineSimplexer(a, b)
...
AdaptiveParameters(::Type{T}=Float64; α = one(T), β = one(T), γ = T(0.75) , δ = one(T)) where T = AdaptiveParameters(α, β, γ, δ)
...
FixedParameters(::Type{T}=Float64; α = T(1), β = T(2), γ = T(0.5), δ = T(0.5)) where T = FixedParameters(α, β, γ, δ)
